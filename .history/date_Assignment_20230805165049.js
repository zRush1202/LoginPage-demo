            ...
            if (mlTime/(1000*60*60) >= 24){
                if (mlTime/(1000*60*60*24) > 30){
                    if (mlTime/(1000*60*60*24*30) > 12){
                        const nam = `${mlTime/(1000*60*60*24*30*12)} năm trước`;
                        return nam;
                    }
                    else{
                        const thang = `${mlTime/(1000*60*60*24*30)} tháng trước`;
                        return thang;
                    }
                }
                else {
                    const ngay = `${mlTime/(1000*60*60*24)} ngày trước`;
                    return ngay;
                }
            }
            else {
                const gio = `${mlTime/(1000*60*60)} giờ trước`;
                return gio;
            }
        }
        else{
            const phut = `${mlTime/(1000*60)} phút trước`;
            return phut;
        }
    }
    else{
        const giay = `${mlTime/1000} giây trước`;
        return giay;
    }
}
...
const timefrom = timeSince(tlnow - tlTime);
console.log(`${timefrom} phút trước`);